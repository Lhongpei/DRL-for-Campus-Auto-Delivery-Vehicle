...
from milp_code.data import Location, Box
def power(lis):
    s = list(lis)
...
def rand_create_Boxes(ids,loc_num,numOfBox):#prefer is 1
    ...
    for j,i in enumerate(ids):
        setofloc=[t for t in range(loc_num)]
        setofloc.remove(i)
        lis=np.random.choice([a for a in setofloc],numOfBox).tolist()
...
def gener_map_list(loc_list):
    location=[]

    for i in range(len(loc_list)):
        location.append(Location(i,loc_list[i][0],loc_list[i][1]))
    return location