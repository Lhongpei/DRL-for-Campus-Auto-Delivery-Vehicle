...
def create_new_env():
    return WeightedObsGrid(
        ...
        weights=None #np.random.rand(grid_size[0], grid_size[1])
    )
...
with tqdm(total=int(num_episodes), desc='Iteration ' ) as pbar:
    for i_episode in range(int(num_episodes)):
        ...
        while (not done) and (iter_num < max_steps):  # 添加最大步数限制
            ...
            if done:
                replay_buffer.add(trajectory['states'], trajectory['actions'], trajectory['rewards'],
                                  trajectory['next_states'], trajectory['dones'])
